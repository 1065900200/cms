...
class DbManage {
     ...
     */
    function __construct($host = 'localhost', $username = 'root', $password = 'root', $database = 'sycms', $charset = 'utf8') {
    ...
    function backup($tablename = '', $dir, $size) {
        ...
        if (! empty ( $tablename )) {
            ...
            $this->_showMsg("恭喜您! <span class='imp'>备份成功</span> <p><a href='/pskcms/Database/backup' style='color:#f60;'>返回</a></p>");
		    $this->_showMsg('<script>setTimeout(function(){ window.location.href = "/pskcms/Database/backup"; },1000)</script>');
        }
    }
    ...
    private function _showMsg($msg,$err=false){
        ...
        echo "<div style='color:#666;'><p class='dbDebug'>".$err . $msg."</p></div>";
        flush();
        ...
        if (empty ( $volume [1] )) {
            ...
            while ( $volume_id ) {
                ...
                if (file_exists ( $tmpfile )) {
					...
					$this->_showMsg('<script>setTimeout(function(){ window.location.href = window.history.go(-1); },1000)</script>');
                    return;
                }
                $volume_id ++;
            }
