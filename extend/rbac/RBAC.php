...
class RBAC {
    ...
    static public function getAccessList($authId) {
        ...
        foreach($apps as $key=>$app) {
            ...
            $sql    =   "select node.id,node.name from ".
                    ...
                    "where user.id='{$authId}' and user.role=role.id and ( access.role_id=role.id  or (access.role_id=role.pid and role.pid!=0 ) ) and role.status=1 and access.node_id=node.id and node.level=2 and node.status=1";
	...
	static public function getModuleAccessList($authId,$module) {
        ...
        $sql    =   "select access.node_id from ".
                    ...
                    $table['user']." as user,".
                    $table['access']." as access ".
                    "where user.id='{$authId}' and user.role=role.id and ( access.role_id=role.id  or (access.role_id=role.pid and role.pid!=0 ) ) and role.status=1 and  access.module='{$module}' and access.status=1";
        $rs =   $db->query($sql);
        $access	=	array();
        foreach ($rs as $node){
            $access[]	=	$node['node_id'];
