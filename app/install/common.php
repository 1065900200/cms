...
function check_dirfile(){
	$items = array(
		...
		array('dir',  '可写', 'success', 'public/backup/'),
	);
...
function create_tables($db, $prefix = ''){
	//读取SQL文件
	$sql = file_get_contents(APP_PATH . 'install/data/sql.sql');
	$sql = str_replace("\r", "\n", $sql);
	$sql = explode(";\n", $sql);
...
function register_administrator($db, $prefix, $admin){
	...
	$sql = "update `[PREFIX]user` set password='[PASS]' where username='admin';";	
	$sql = str_replace(array('[PREFIX]','[PASS]'),array($prefix,$password),$sql);	
	$db->execute($sql);
	show_msg('创始人帐号注册完成！');
}
...
function update_tables($db, $prefix = ''){
	...
	//替换表前缀
	$sql = str_replace(" `sy_", " `{$prefix}", $sql);
